...
public class ImageListModel {
    ...
    public static ImageListModel generateAllByProjIdAndUsername(String proejectId, String username){
        ImageListModel imageListModel = new ImageListModel();
        imageListModel.setImageList(ImageModel.listImagesByProjectIdAndUsername(proejectId, username));
        return imageListModel;
    }
    ...
    public int estimateAndClose(WorkerModel wmodel, int[] creditGrade, MarkTypeEnum type){
        int credit = 0;
        for (ImageModel model: this.imageList){
            if (model.markedByMarker(wmodel.getUsername())){
                credit += model.estimateMark(wmodel, creditGrade, type);
            }
            else {
                model.deleteMarker(wmodel.getUsername());
            }
        }
        closeImages(wmodel.getUsername());
        return credit;
    }


    public List<Integer> extractEveryWorkerCredit(String username){
        List<Integer> credits = this.imageList.stream().map(e->{
            int credit = e.queryWorkerCreditByUsername(username);
