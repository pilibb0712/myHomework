...
public class NewPurchaseBillUiController {
	...
	public void init(UserInfoVO user, boolean come) {
		...
		try {
			...
			ArrayList<PurchaseBillVO> bills;
			if(service.getPurchaseDraftBillsList(user)!=null) {
				bills=service.getPurchaseDraftBillsList(user);
			}else {
				bills=null;
			}
			...
			draftSubmitColumn.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<BillVO,Button>, ObservableValue<Button>>() {
				...
				public ObservableValue<Button> call(CellDataFeatures<BillVO, Button> param) {
					...
					submit.setOnAction(paramx->{
						try {
							service.SubmitPurchase(bill.getId());
							drafts.clear();
							ArrayList<PurchaseBillVO> bills;
							if(service.getPurchaseDraftBillsList(userInfoVO)!=null) {
								bills=service.getPurchaseDraftBillsList(userInfoVO);
							}else {
								bills=null;
							}
